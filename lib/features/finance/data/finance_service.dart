...
class FinanceService {
  ...
  Future<FinanceResult> recordManual({
    ...
    return _db.transaction(() async {
      ...
      await _db.into(_db.ledgerEntries).insert(LedgerEntriesCompanion.insert(
            ...
            amountMinor: amountMinor,
            note: Value(note ?? category ?? (isIncome ? 'Pemasukan' : 'Pengeluaran')),
          ));

      await (_db.update(_db.accounts)..where((t) => t.id.equals(accountId)))
          .write(AccountsCompanion(
        currentBalanceMinor: Value(acc.currentBalanceMinor +
  ...
  Future<FinanceResult> transfer({
    ...
    return _db.transaction(() async {
      final from = await (_db.select(_db.accounts)
            ..where((t) => t.id.equals(fromAccountId)))
          .getSingle();
      final to = await (_db.select(_db.accounts)
      ...
      await (_db.update(_db.accounts)
            ..where((t) => t.id.equals(fromAccountId)))
          .write(AccountsCompanion(
        currentBalanceMinor: Value(from.currentBalanceMinor - amountMinor),
      ));
      ...
      await (_db.update(_db.accounts)..where((t) => t.id.equals(toAccountId)))
          .write(AccountsCompanion(
  ...
  Future<List<LedgerEntry>> cashFlow(int businessId, {int limit = 100}) async {
    final accountIds =
        await (_db.select(_db.accounts)..where((t) => t.businessId.equals(businessId)))
            .map((a) => a.id)
            .get();
