...
class _StockScreenState extends ConsumerState<StockScreen> {
  ...
  Future<void> _openActionSheet(Product p, _StockAction action) async {
    ...
    final label = switch (action) {
      _StockAction.opening =>
        'Stok awal (${p.type == 'goods' ? 'pcs dasar' : ''})',
    ...
    final confirmed = await showModalBottomSheet<bool>(
      ...
      builder: (sheetContext) => Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              '${_actionName(action)} — ${p.name}',
              style: Theme.of(sheetContext).textTheme.titleMedium,
            ),
            const SizedBox(height: 4),
            Text(
              'Stok saat ini: ${microToDecimalString(p.stockQuantityMicro)}',
    ...
    final r = switch (action) {
      ...
      _StockAction.adjustIn => await service.adjustStock(
        p.id,
        qtyMicro.abs(),
        'Penyesuaian tambah',
      ),
      _StockAction.adjustOut => await service.adjustStock(
        p.id,
        -qtyMicro.abs(),
        'Penyesuaian kurang',
      ),
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          'Stok sekarang ${microToDecimalString(r.newStockMicro!)}',
        ),
      ),
    );
