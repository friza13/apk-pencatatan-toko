...
class ReceivableService {
  ...
  Future<List<ReceivableWithCustomer>> list(int businessId) async {
    final rows = await (_db.select(_db.receivables).join([
      innerJoin(_db.customers,
          ...
          ..orderBy([
            OrderingTerm.asc(_db.receivables.dueDate),
          ]))
        .get();
    return rows
        .map((r) => (
  ...
  Future<FinanceResult> recordPayment({
    ...
    if (amountMinor <= 0) {
      return const FinanceResult.failure(Failure(
          code: ErrorCodes.invalidPayment, message: 'Nominal harus > 0'));
    }

    return _db.transaction(() async {
      final r = await (_db.select(_db.receivables)
            ..where((t) => t.id.equals(receivableId)))
          .getSingle();

      if (r.remainingAmountMinor <= 0) {
        return const FinanceResult.failure(
            Failure(code: ErrorCodes.invalidPayment, message: 'Piutang sudah lunas.'));
      }
      if (amountMinor > r.remainingAmountMinor) {
        return FinanceResult.failure(Failure(
      ...
      // Allocation junction.
      await _db.into(_db.receivablePayments).insert(
      ...
      await (_db.update(_db.receivables)
        ...
        closedAt: fullyPaid ? Value(DateTime.now().toUtc()) : const Value(null),
      ));
...
typedef ReceivableWithCustomer = ({
