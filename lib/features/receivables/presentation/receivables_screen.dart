...
class _ReceivablesScreenState extends ConsumerState<ReceivablesScreen> {
  ...
  bool _isOverdue(Receivable r, DateTime nowUtc) =>
      r.remainingAmountMinor > 0 && r.dueDate.isBefore(nowUtc);
  ...
  List<ReceivableWithCustomer> get _filtered {
    ...
    switch (_segment) {
      ...
      case _Segment.overdue:
        return _items.where((i) => _isOverdue(i.receivable, now)).toList();
      case _Segment.paid:
        return _items
            .where((i) => i.receivable.remainingAmountMinor <= 0)
            .toList();
  ...
  Future<void> _pay(ReceivableWithCustomer item) async {
      ...
      builder: (sheetContext) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              '${item.customerName} — sisa ${formatMinor(r.remainingAmountMinor)}',
            ),
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          result.isSuccess ? 'Pembayaran dicatat.' : result.failure!.message,
        ),
      ),
    );
    await _reload();
