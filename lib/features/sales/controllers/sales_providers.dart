...
final FutureProvider<int> defaultAccountIdProvider =
  ...
  final rows = await (db.select(db.accounts)
      ...
      .get();
  if (rows.isNotEmpty) {
    return rows.firstWhere((a) => a.type == 'cash', orElse: () => rows.first).id;
  }
...
class CartLine {
  ...
  int get lineTotalMinor =>
      divideHalfUpRound(qtyMicro * unitPriceMinor);
}

int divideHalfUpRound(int numerator) =>
    (numerator + 500000) ~/ 1000000;
...
class CartState {
  ...
  int get subtotalMinor =>
      lines.fold(0, (sum, l) => sum + l.lineTotalMinor);
...
class CartController extends Notifier<CartState> {
  ...
  void add(Product p) {
    final existing =
        state.lines.where((l) => l.productId == p.id).toList();
  ...
  void _changeQty(int productId, int newQtyMicro) {
    if (newQtyMicro <= 0) {
      state = CartState(
          lines:
              state.lines.where((l) => l.productId != productId).toList());
      return;
    }
    state = CartState(lines: [
  ...
  void increment(int productId) {
    final line =
        state.lines.where((l) => l.productId == productId).firstOrNull;
    if (line == null) return;
...
final FutureProvider<List<Sale>> salesListProvider =
    FutureProvider<List<Sale>>((ref) async {
  final business = await ref.watch(currentBusinessProvider.future);
  final db = await ref.watch(appDatabaseProvider.future);
...
final saleDetailProvider =
    FutureProvider.family<({Sale sale, List<SaleLine> lines})?, int>(
        (ref, id) async {
  final db = await ref.watch(appDatabaseProvider.future);
  final sale = await (db.select(db.sales)..where((t) => t.id.equals(id)))
      .getSingleOrNull();
