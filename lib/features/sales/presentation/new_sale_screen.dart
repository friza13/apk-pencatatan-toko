...
class _NewSaleScreenState extends ConsumerState<NewSaleScreen> {
  ...
  Future<void> _runSearch(String q) async {
    ...
    final items = await repo.search(
        businessId: business.id, query: q, filter: ProductFilter.active);
    if (!mounted) return;
    setState(() {
      _results = items;
      _searching = false;
  ...
  Future<void> _checkout() async {
    ...
    final confirmed = await showModalBottomSheet<bool>(
      ...
      builder: (sheetContext) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Text('Pembayaran', style: Theme.of(sheetContext).textTheme.titleMedium),
          const SizedBox(height: 4),
          Text('Total: ${formatMinor(cart.subtotalMinor)}',
          ...
          TextField(
            ...
            decoration:
                const InputDecoration(labelText: 'Dibayar sekarang'),
          ),
          const SizedBox(height: 8),
          Text('Kurang dari total akan dicatat sebagai piutang.',
    ...
    final paidNow = int.tryParse(paidC.text.replaceAll(RegExp(r'[^0-9]'), '')) ?? 0;

    try {
      final service = await ref.read(salesServiceProvider.future);
      final out = await service.checkout(CheckoutInput(
        lines: [
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 12),
                      const Text('Cari produk di atas,\nlalu ketuk untuk menambah.'),
                    ],
                  ),
                )
              : ListView(
        ...
        child: Padding(
          ...
          child: FilledButton(
            style: FilledButton.styleFrom(
              ...
              backgroundColor:
                  cart.isEmpty ? Theme.of(context).disabledColor : null,
            ),
