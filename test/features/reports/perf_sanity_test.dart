...
void main() {
  ...
  test('bulk insert 10k products lalu search tetap cepat', () async {
    await db.into(db.units).insert(UnitsCompanion.insert(businessId: businessId, code: 'pcs', name: 'Pieces'));
    final pcsId = (await (db.select(db.units)
    ...
    final hits = await repo.search(businessId: businessId, query: 'Produk 9999');
    t1.stop();
    // ignore: avoid_print
    print('[PERF] search exact: ${t1.elapsedMilliseconds} ms, '
        'hits=${hits.length}');

    final t2 = Stopwatch()..start();
    await repo.search(businessId: businessId, query: '99', filter: ProductFilter.active);
    t2.stop();
    // ignore: avoid_print
    ...
    expect(t2.elapsedMilliseconds, lessThan(2000),
