...
void main() {
  ...
  Future<void> backdateLastSaleByDays(int days) async {
    ...
    await (db.update(db.sales)..where((t) => t.id.equals(sale.id))).write(
      SalesCompanion(
        createdAt: Value(DateTime.now().toUtc().subtract(Duration(days: days))),
      ),
    );
  }
  ...
  test('backdated sale counted in last7 but not today', () async {
    await SalesService(db).checkout(
      CheckoutInput(
        lines: [
          SaleLineInput(
            productId: barangId,
  ...
  test('voided sale excluded from summaries', () async {
    ...
    await (db.update(db.sales)..where((t) => t.id.equals(out.saleId))).write(
      const SalesCompanion(status: Value('voided')),
    );
  ...
  test('stockValuation uses cached stock x WAC', () async {
    ...
    expect(
      await reports.stockValuationMinor(businessId),
      376000000 ~/ 1000 * 1000 == 0 ? 0 : 47 * 8000,
    );
  });
