...
void main() {
  ...
  test('paid sale commits everything atomically', () async {
    ...
    final acc = await (db.select(db.accounts)
          ..where((t) => t.id.equals(cashAccountId)))
        .getSingle();
  ...
  test('credit sale creates receivable; partial payment marks partially_paid',
      () async {
    final out = await sales.checkout(CheckoutInput(
  ...
  test('voidSale restores stock and writes audit; status voided', () async {
    final out = await sales.checkout(CheckoutInput(
    ...
    await sales.voidSale(out.saleId, 'salah input');
    expect(await stockOf(barangId), 20000000);

    final sale = await (db.select(db.sales)).getSingle();
    expect(sale.status, 'voided');
    final logs = await (db.select(db.activityLogs)
          ..where((t) => t.action.equals('sale.voided')))
        .get();
    expect(logs, hasLength(1));
  });

  test('invoice numbers increment across sales', () async {
    final a = await sales.checkout(CheckoutInput(
      lines: [line(jasaId, '1', 5000)],
      accountId: cashAccountId,
      paidNowMinor: 5000,
    ));
    final b = await sales.checkout(CheckoutInput(
      lines: [line(jasaId, '1', 5000)],
      accountId: cashAccountId,
      paidNowMinor: 5000,
    ));
